...
import type { ReportedTutor } from "@spt/types/report";
...
const TableBody: FC<ComponentProps> = ({ items, currentPage, pageSize }) => {
  ...
  const handleViewMore = (item: ReportedTutor) => {
    navigate(
      generatePath(routes.main.reports.reportedTutors.details, { id: item.id }),
      { state: { report: item } }
    );
  };
